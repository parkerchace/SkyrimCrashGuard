...
StateValidationResult StateManager::ValidateState() {
    ...
    if (tes) {
        ...
        // CommonLibSSE NG moved worldSpace out of TES's fixed members into the
        // versioned RUNTIME_DATA2 block (its offset moved in AE 1.6.1130);
        // interiorCell is still a plain member.
        if (!tes->GetRuntimeData2().worldSpace && !tes->interiorCell) {
